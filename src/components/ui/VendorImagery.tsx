...
export default function VendorImagery({ images = [], category, name }: { images?: string[]; category: string; name: string }) {
  ...
  return <div className="relative h-52 overflow-hidden bg-paper-dim">
    ...
    <span className="absolute bottom-3 left-3 rounded-full bg-white/90 px-2.5 py-1 text-[10px] font-medium text-navy">{fallback ? 'Illustrative photo' : 'From vendor website'}</span>
    {valid.length > 1 && <button type="button" aria-label={`Next photo for ${name}`} onClick={() => setSelected(value => value + 1)} className="absolute bottom-3 right-3 rounded-full bg-white/90 px-3 py-1 text-xs text-navy">{selected % valid.length + 1} / {valid.length} →</button>}
  </div>;
}
